...
class Create extends Component
{
    ...
    {
        return [
            'identify' => 'required|string|unique:lecturers,id',
    ...
    public function store()
    {
        ...
        try {
            ...
            $newUser = Lecturer::create($newUser);
            ...
            DB::commit();
            $this->dispatch('refreshLecturers');
            $this->dispatch('setSearch', $this->name);
            $this->dispatch('toggle-create-lecturer-modal');
            $this->refresh();
            $this->alert('success', __('Successfully'), ['text' => __(':attribute created successfully.', ['attribute' => __('Lecturer')])]);
        } catch (\Exception $e) {
            DB::rollBack();
